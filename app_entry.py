...
# ── DATA — NO CACHE ON INSTRUMENTS (must be always fresh for lot size) ────────
...
def read_instruments():
    # NO CACHE — must always be fresh so lot size lookup works immediately after upload
    try:
...
# ── SESSION STATE ─────────────────────────────────────────────────────────────
for k,v in [("edit_id",None),("symbol",""),("instrument","FUT"),
             ("exchange","NSE"),("lots",1)]:
...
st.markdown(f'<div style="font-size:0.72rem;color:{col_status};margin-bottom:6px;">📋 {inst_count} F&O symbols loaded{" · " + inst_last if inst_last else " — upload fo_mktlots.csv below"}</div>', unsafe_allow_html=True)
...
# ══════════════════════════════════════════════════════
# LEFT — ENTRY
# KEY DESIGN: Symbol/Instrument/Exchange selectors are
# OUTSIDE the form so they trigger immediate reruns.
# Only price/notes/submit are inside the form.
# ══════════════════════════════════════════════════════
with col_form:
    ...
    if not strategies:
        ...
        # ── SELECTORS OUTSIDE FORM (trigger immediate rerun on change) ──
        ...
        # ── LOT SIZE — computed immediately on every rerun ──
        lot_size = 1
        ...
        # ── QTY DISPLAY — always shows current values ──
        if instrument != "CASH":
            ...
            if lot_size == 1 and symbol:
                st.markdown(f'<div class="warn-box">⚠️ <b>{symbol}</b> not found in {inst_count} loaded symbols.</div>', unsafe_allow_html=True)

        st.markdown("<div style='height:8px'></div>", unsafe_allow_html=True)

        # ── FORM — only price/date/notes/submit ──
            ...
            if not manual and not is_back and instrument != "CASH":
                try:
                    ...
                    if ltp and ltp > 0:
                        ...
                        st.markdown('<div class="warn-box">AngelOne returned no price.</div>', unsafe_allow_html=True)
...
with col_trades:
    ...
    if not df.empty:
        ...
        m2.markdown(f'<div class="stat-box"><div class="stat-label">MTM P&L</div><div class="stat-value" style="color:{mtm_c}">{mtm_s}</div><div class="stat-sub">{"live" if ltp_map else "add AngelOne"}</div></div>', unsafe_allow_html=True)
    ...
    if df.empty:
        ...
        for _,row in df.iterrows():
            ...
            # Qty — use CURRENT lot size from instruments
            cur_ls  = get_lot_size(sym)
            cur_qty = int(lv)*cur_ls if (str(lv).isdigit() and cur_ls>1) else qv
            if instr != "CASH":
                qty_info = f"{lv} lots × {cur_ls} = {cur_qty} sh"
            else:
                qty_info = f"{qv} shares"

            # LTP + per-trade MTM
            ltp_line = ""
            if sym in ltp_map:
                lv_price = ltp_map[sym]
                try:
                    ep   = float(pv)
                    qty  = float(qv)
                    pnl  = (lv_price-ep)*qty if act.upper()=="BUY" else (ep-lv_price)*qty
                    pc   = "#22c55e" if pnl>=0 else "#ef4444"
                    sign = "+" if pnl>=0 else ""
                    ltp_line = (f' &nbsp;·&nbsp; <span style="color:#94a3b8">LTP {fmt_px(lv_price)}</span>'
                                f' <span style="color:{pc};font-weight:600">{sign}₹{abs(pnl):,.0f}</span>')
                except:
                    ltp_line = f' &nbsp;·&nbsp; <span style="color:#94a3b8">LTP {fmt_px(ltp_map[sym])}</span>'
            ...
            with ci:
                st.markdown(
                    ...
                    f'<div class="trade-meta">{dt_v} · {fmt_px(pv)}</div>',
                    unsafe_allow_html=True
                )
            with cb2:
                b1,b2 = st.columns(2)
