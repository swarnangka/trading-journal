        ...
        with st.form("entry_form", clear_on_submit=True):
            ...
            if instrument != "CASH":
                ...
                if lot_size == 1 and symbol:
                    st.markdown(
                        f'<div class="warn-box">⚠️ <b>{symbol}</b> not in instruments list. '
                        f'Upload fo_mktlots.csv → Update Instruments, then come back.</div>',
                        unsafe_allow_html=True
                    )
...
with col_trades:
    ...
    if df.empty:
        ...
        for _, row in df.iterrows():
            ...
            if instr != "CASH":
                qty_info = f"{lots_v} lots × {ls_v} = {qty_v} sh"
            else:
