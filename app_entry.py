...
def get_ws(tab):
    """Get worksheet — always fresh to ensure writes go through."""
                ...
                for _, row in df.iterrows():
                    ...
                    if sym in ltp_map:
                        ...
                        if instr.upper() in ("CASH","ETF"):
                            # Full capital deployed = qty × LTP (100% margin for delivery)
                            cash_qty   = float(row.get("quantity", 0) or 0)
                            pos_margin = ltp * cash_qty   # 100% of position value
                        else:
