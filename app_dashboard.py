...
def fetch_ltp_batch(positions):
    """Fetch LTP for multiple symbols using core.angelone. Returns {symbol: ltp}."""
    ltp_map = {}
    try:
        from core.angelone import get_angel_obj as _ao_fn, get_symbol_token, fetch_current_ltp
        ao = _ao_fn()
        if not ao: return {}
        for pos in positions:
            ...
            exp   = pos.get("expiry","")
            stk   = float(pos.get("strike", 0) or 0)
            opt   = pos.get("option_type","")
            if sym and sym not in ltp_map:
                token = get_symbol_token(sym, exch, instr, exp, stk, opt)
                ltp, _ = fetch_current_ltp(sym, exch, token, instr)
                if ltp and ltp > 0:
                    ltp_map[sym] = ltp
    except Exception as e:
        st.warning(f"AngelOne connection error: {e}")
    return ltp_map
...
if df_open.empty:
    ...
    positions_list = [
        {"symbol":       str(r.get("symbol","")),
         ...
         "instrument":   str(r.get("instrument","FUT")),
         "expiry":       str(r.get("expiry","")),
         "strike":       str(r.get("strike","0") or "0"),
    ...
    for _, row in df_open.iterrows():
        ...
        if sym in ltp_map:
            ltp = ltp_map[sym]
            if instr.upper() in ("CASH","ETF"):
                qty = float(row.get("quantity",0) or 0)
                margin_map[tid] = ltp * qty  # 100% capital deployed
            else:
                ls  = get_lot_size(sym)
